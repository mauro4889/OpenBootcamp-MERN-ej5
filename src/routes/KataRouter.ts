...
import { getKataByID } from "@/domain/orm/Kata.orm";
...
katasRouter.route('/')
    ...
    .put(jsonParser, verifyToken, async (req: Request, res: Response) => {
        ...
        if (name && description && level && intents >= 0 && stars >= 0 && creator && solution && participants.length >= 0) {
            ...
            let data = await getKataByID(id)

            if (data) {
                let mediaStars = (data.stars + stars) / 2
...
katasRouter.get('/getRecent', async (req: Request, res: Response) => {
    const controller: KatasController = new KatasController();
    // Obtain Reponse
...
katasRouter.get('/getBestKatas', async (req: Request, res: Response) => {
    const controller: KatasController = new KatasController()
    // Obtain Reponse
    const response: any = await controller.getRecentKatas()
    // Send to the client the response
...
katasRouter.get('/getBestKatas', async (req: Request, res: Response) => {
    const controller: KatasController = new KatasController()
    // Obtain Reponse
    const response: any = await controller.getBestkatas()
    // Send to the client the response
    return res.status(200).send(response);
})

katasRouter.get('/getKatasByIntent', async (req: Request, res: Response) => {
    const controller: KatasController = new KatasController()
    // Obtain Reponse
    const response: any = await controller.getKatasIntent()
    // Send to the client the response
    return res.status(200).send(response);
})
