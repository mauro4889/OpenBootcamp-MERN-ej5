...
import { getAllKatas, getKataByID, updateKataByID, deleteKataByID, createKata, getKatasFilter, getKatasRecent, getKatasOrderByBest } from "../domain/orm/Kata.orm";
...
export class KatasController implements IKataController {
     ...
     */
    @Get("/")
    public async getKatas(@Query() page: number, @Query() limit: number, @Query() id?: string, @Query() level?: any): Promise<any> {

        let response: any = '';

        if (level) {
            LogSuccess(`[/api/katas] Get Kata By Filter: ${level} `);
            response = await getKatasFilter(level)
        }
    ...
    @Get("/getRecent")
    public async getRecentKatas() {

        let response: any = ''
        LogSuccess(`[/api/katas/getRecent] Get Kata recent`);
    ...
    @Get("/getBestKatas")
    public async getBestkatas() {

        let response: any = ''

        LogSuccess(`[/api/katas/getBestKatas] Get Best Katas`);

        response = await getKatasOrderByBest()

        return response

    }

    @Get("/getKatasByIntent")
    public async getKatasIntent(){
        let response: any = ''

        LogSuccess(`[/api/katas/getKatasByIntent] Get Katas By Intent`);

        response = await this.getKatasIntent()
